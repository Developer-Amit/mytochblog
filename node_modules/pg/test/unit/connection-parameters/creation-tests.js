...
test('ConnectionParameters construction', function() {
  ...
  assert.ok(new ConnectionParameters('postgres://192.168.1.114/postgres'), 'with connection string');
});
...
test('escape spaces if present', function() {
  subject = new ConnectionParameters('postgres://192.168.1.114/post gres');
  assert.equal(subject.database, 'post gres');
});

test('do not double escape spaces', function() {
  subject = new ConnectionParameters('postgres://192.168.1.114/post%20gres');
  assert.equal(subject.database, 'post gres');
});
...
test('libpq connection string building', function() {
  ...
  test('builds simple string', function() {
    ...
    subject.getLibpqConnectionString(assert.calls(function(err, constring) {
      ...
      checkForPart(parts, "hostaddr=192.168.1.114");
  ...
  test('username or password contains weird characters', function() {
    ...
    defaults.ssl = true;
    var strang = 'pg://my f%irst name:is&%awesome!@192.168.1.114:9000';
    ...
    assert.equal(subject.host, '192.168.1.114');
    assert.equal(subject.ssl, true);
  });

  test("url is properly encoded", function() {
    var encoded = "pg://bi%25na%25%25ry%20:s%40f%23@192.168.1.114/%20u%2520rl";
